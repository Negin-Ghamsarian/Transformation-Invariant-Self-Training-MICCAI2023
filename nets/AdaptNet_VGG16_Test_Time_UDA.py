...
class For_Discriminator(nn.Module):
    def __init__(self):
        super(For_Discriminator, self).__init__()

        self.Interpolation1 = nn.Upsample(scale_factor=8)
        self.Interpolation2 = nn.Upsample(scale_factor=4)
        self.Interpolation3 = nn.Upsample(scale_factor=2)

    def forward(self, x1, x2, x3, x4):  

        x1 = self.Interpolation1(x1)
        x2 = self.Interpolation2(x2)
        x3 = self.Interpolation3(x3)

        concatenated = torch.cat([x1, x2, x3, x4], dim = 1)

        return concatenated

class AdaptNet_VGG16_TTUDA(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(AdaptNet_VGG16_TTUDA, self).__init__()
    ...
    def forward(self, x):
        ...
        D_Features = self.For_D(x1, x2, x3, x4)
...
if __name__ == '__main__':
    model = AdaptNet_VGG16_TTUDA(n_channels=3, n_classes=1)
