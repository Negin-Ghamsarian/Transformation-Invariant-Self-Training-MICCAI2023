...
class scSE_Net_VGG16_WithRepHead(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        super(scSE_Net_VGG16_WithRepHead, self).__init__()
        ...
        self.representation = nn.Sequential(
                nn.Conv2d(512, 8, kernel_size=1, stride=1, padding=0, bias=True),
                nn.Flatten(),
    ...
    def forward(self, x):
        ...
        representation_head = self.representation(out1)


        return logits, representation_head

    
    
    
if __name__ == '__main__':
    model = scSE_Net_VGG16_WithRepHead(n_channels=3, n_classes=3)
    print(model)
    template = torch.ones((1, 3, 512, 512))
    
    
    y1, representation_head = model(template)
    print(y1.shape)
    print(representation_head.shape)
    # model.cuda()
